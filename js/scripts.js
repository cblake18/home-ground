...
// Initialize Perlin noise when DOM is ready
function initPerlinClouds() {
    // Check if canvases exist, if not create them
    let heroSection = document.getElementById('home');
    let contactSection = document.getElementById('contact');
    
    // Create canvases if they don't exist
    if (heroSection && !document.getElementById('hero-fractal')) {
        const heroCanvas = document.createElement('canvas');
        heroCanvas.id = 'hero-fractal';
        heroCanvas.className = 'fractal-canvas';
        heroSection.insertBefore(heroCanvas, heroSection.firstChild);
    }
    
    if (contactSection && !document.getElementById('contact-fractal')) {
        const contactCanvas = document.createElement('canvas');
        contactCanvas.id = 'contact-fractal';
        contactCanvas.className = 'fractal-canvas';
        contactSection.insertBefore(contactCanvas, contactSection.firstChild);
    }
    
    // Now initialize the canvases
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPerlinClouds);
} else {
    ...
    initPerlinClouds();
            ...
            window.addEventListener('resize', () => {
                if (heroFractal) {
                    heroFractal.width = heroFractal.offsetWidth;
                    heroFractal.height = heroFractal.offsetHeight;
                }
                if (contactFractal) {
                    contactFractal.width = contactFractal.offsetWidth;
                    contactFractal.height = contactFractal.offsetHeight;
                }
            });
            ...
            keyElements.forEach(key => {
                ...
                key.addEventListener('mouseup', () => this.classList.remove('active'));
                key.addEventListener('mouseleave', () => this.classList.remove('active'));
            });
